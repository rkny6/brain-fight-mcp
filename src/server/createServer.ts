...
const SERVER_INSTRUCTIONS =
  ...
  "MILESTONES: end_inner_conflict may return milestones (first time cooperation gets very high, a 5-debate win streak, the 10th debate in a domain). Each fires ONCE EVER per (session, domain) — when performance_instructions calls for it, let Angel and Devil react in one brief in-character line each, genuinely surprised, not a fourth-wall explanation. This only happens because of real accumulated history; don't manufacture similar moments yourself when milestones is empty. " +
